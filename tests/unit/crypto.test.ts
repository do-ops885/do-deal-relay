...
import { sha256, generateDealId, generateSnapshotHash, generateUUID, calculateStringSimilarity, calculateUrlSimilarity } from '../worker/lib/crypto';
...
describe('Crypto Utilities', () => {
  ...
  describe('calculateUrlSimilarity', () => {
    it('should return 1.0 for identical URLs', () => {
      expect(calculateUrlSimilarity('https://example.com/path', 'https://example.com/path')).toBe(1.0);
    });

    it('should return 0.0 for different domains', () => {
      expect(calculateUrlSimilarity('https://example.com', 'https://other.com')).toBe(0.0);
    });
  });
});
