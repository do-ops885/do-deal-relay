...
describe("SourceRegistry", () => {
  ...
  describe("consumeRateLimit", () => {
    it("should atomically limit concurrent requests for one actor key", async () => {
      ...
      const exec = vi.fn((sql: string, ...params: unknown[]) => {
        ...
        if (
          ...
          const state = rateLimits.get(String(params[0]));
          return cursor(
            state
              ? [{ count: state.count, window_start: state.windowStart }]
              : [],
          );
        }
        if (normalizedSql.startsWith("INSERT INTO RATE_LIMITS")) {
          rateLimits.set(String(params[0]), {
            ...
            windowStart: Number(params[2]),
          });
        }
        return cursor([]);
      });
      const state = {
        id: { name: "rate-limit-actor" },
        storage: { sql: { exec } },
      } as unknown as DurableObjectState;
      const registry = new SourceRegistry(state);
      ...
      const results = await Promise.all(
        Array.from({ length: 20 }, () =>
          registry.consumeRateLimit("client-window", 10, 1_700_000_000),
        ),
      );

      expect(results.filter((result) => result.allowed)).toHaveLength(10);
      expect(results.filter((result) => !result.allowed)).toHaveLength(10);
      expect(rateLimits.get("client-window")?.count).toBe(10);
    });
    ...
    it("should reset a stable rate-limit key when the window changes", async () => {
      ...
      await registry.consumeRateLimit("stable-key", 1, 60);
      expect(
        (await registry.consumeRateLimit("stable-key", 1, 60)).allowed,
      ).toBe(false);
      expect(
        (await registry.consumeRateLimit("stable-key", 1, 120)).allowed,
      ).toBe(true);
      expect(states.get("stable-key")).toEqual({ count: 1, windowStart: 120 });
    });
  });
