...
# Validation guards live in scripts/pre-commit-validation.sh (sourced).
# shellcheck source=pre-commit-validation.sh
SCRIPT_HOOK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "${SCRIPT_HOOK_DIR}/pre-commit-validation.sh" ]; then
    # shellcheck disable=SC1091
    source "${SCRIPT_HOOK_DIR}/pre-commit-validation.sh"
else
    # Installed as .git/hooks/pre-commit (copied): resolve via git toplevel.
    HOOK_TOPLEVEL="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
    # shellcheck disable=SC1091
    source "${HOOK_TOPLEVEL}/scripts/pre-commit-validation.sh"
fi
run_syntax_guards
