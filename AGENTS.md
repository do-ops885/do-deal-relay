...
- **Fix-Forward**: Always attempt to fix pre-existing issues and warnings encountered during work. If a fix is not possible (e.g., upstream bug, external dependency, out-of-scope), create a follow-up plan as `plans/FOLLOWUP-<brief-slug>.md` documenting the issue and proposed resolution.
