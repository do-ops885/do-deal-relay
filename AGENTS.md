...
**Version**: 0.2.2 (Adapted Upstream Workflow Standards v0.3.5)

## 1. Named Constants & Hot Files
```bash
...
## 2. Analyze-First Mandate
1. **Analyze First**: Prior to asking ANY clarification questions, deeply analyze the repository structure, local tooling, and workflows.
...
3. **Infer Conventions**: Align with existing patterns (Centralized Middleware, SQLite tables, strict TypeScript) instead of generic template styles.

## 3. Process Modes & PEV Loop (Plan-Execute-Verify)
- **Light Mode** (Small fixes, docs): Quality gate (`./scripts/quality_gate.sh`) → atomic commit → PR.
...
- **Validation Pipeline**: Submissions MUST pass the 9 validation gates in `worker/validation/pipeline.ts`. Speculative rewrites are forbidden.

## 4. Operational Commands & Standards
...
- Context Control & Sub-Agents: Use specialized agents in `.opencode/agents/` or `.claude/agents/` as context firewalls.
- Skills: Canonical skills live in `.agents/skills/`. Load only as needed via `skill <name>` to optimize token budget.
- Re-Verification: Before executing GOAP items, re-verify older assumptions. Apply incremental verification during tasks.

## 5. PR & Commit Standards (Zero Slop)
