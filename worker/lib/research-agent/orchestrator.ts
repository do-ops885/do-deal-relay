import { WebResearchRequest, ReferralResearchResult, ReferralInput, Env } from "../../types";
import {
  ResearchSource,
  RESEARCH_SOURCES,
...
import { getApiKeys, getCachedResults, cacheResults, isCircuitOpen, recordSuccess, recordFailure } from "./orchestrator-utils";
...
export async function executeReferralResearch(
  ...
  const agentId = `agent-${crypto.randomUUID().substring(0, 8)}`;
  const errors: string[] = [];
  ...
  // Logic to determine if we should use real fetching
  const hasApiKeys = !!(apiKeys.productHuntToken || apiKeys.githubToken || (apiKeys.redditClientId && apiKeys.redditClientSecret));
  const useRealFetching = request.options?.use_real_fetching === true ||
                          (request.options?.use_real_fetching !== false && (
                            env.ENVIRONMENT === "production" ||
                            (env as any).RESEARCH_USE_REAL_FETCHING === "true" ||
                            hasApiKeys
                          ));

  if (request.domain) {
    const potentialCodes = generatePotentialCodes(request.domain, request.depth || "quick");
    if (potentialCodes.length > 0) {
      sourcesChecked.push(`known_pattern:${request.domain}`);
      for (const p of potentialCodes) {
        discoveredCodes.push({
          code: p.code,
          url: p.url,
          source: `known_pattern:${request.domain}`,
          discovered_at: new Date().toISOString(),
          reward_summary: p.typicalReward,
          confidence: 0.7,
        });
      }
    }
  ...
  const actualSources = sourcesToUse.includes("all")
    ? RESEARCH_SOURCES.slice(0, CONFIG.RESEARCH_MAX_SOURCES_PER_QUERY || 5)
    : RESEARCH_SOURCES.filter(s => sourcesToUse.includes(s.name));

  for (const source of actualSources) {
    researchPromises.push(
      researchFromSourceParallel(
        source,
        normalizedQuery,
        useRealFetching,
        request.depth,
  ...
  const uniqueCodes = deduplicateCodes(discoveredCodes).slice(0, request.max_results || 50);
  ...
  if (request.domain && env.DEALS_SOURCES) {
    try {
...
async function researchFromSourceParallel(
  ...
  if (!researchRateLimiter.canMakeRequest(source.name)) {
    const waitTime = Math.ceil(researchRateLimiter.getTimeUntilNextWindow(source.name) / 1000);
  ...
  if (cached) {
    discoveredCodes.push(...cached.map(c => ({ ...c, source: `${c.source} (cached)` })));
    return;
  }
  ...
  if (useRealFetching && source.apiConfig) {
    try {
      ...
      if (fetchResult.success) {
        ...
        const extracted = extractReferralsFromContent(fetchResult.content, source, source.name);
        const newCodes: ReferralResearchResult["discovered_codes"] = [];

        for (const referral of extracted) {
          if (referral.confidence >= (CONFIG.RESEARCH_MIN_CONFIDENCE || 0.5)) {
            const codeEntry = {
              code: referral.code,
              url: referral.url,
              ...
              confidence: applySourceConfidence(referral.confidence, source.name),
            };
    ...
    discoveredCodes.push(...simulatedCodes.map(c => ({
      ...c,
...
function applySourceConfidence(baseConfidence: number, sourceName: string): number {
  ...
  for (const discovered of researchResult.discovered_codes) {
    ...
    const referral: ReferralInput = {
      ...
      metadata: {
        ...
        description: discovered.reward_summary || `Referral code discovered via web research`,
        reward_type: "unknown",
    ...
    if (env.DEALS_SOURCES) {
      try {
        await env.DEALS_SOURCES.put(`referral:${id}`, JSON.stringify(referral), { expirationTtl: 2592000 });
      } catch (e) {
