...
export async function handleD1Deals(url: URL, env: Env): Promise<Response> {
  if (!env.DEALS_DB) {
    return jsonResponse({ error: "D1 database not configured" }, 503, undefined, env);
  }
...
export async function handleD1Similar(url: URL, env: Env): Promise<Response> {
  ...
  if (!dealId) {
    return jsonResponse({ error: "deal_id query parameter required" }, 400, undefined, env);
  }

  if (!env.DEALS_DB) {
    return jsonResponse({ error: "Database not configured" }, 503, undefined, env);
  }
  ...
  try {
    ...
    return jsonResponse({ similar: deals, total: deals.length }, undefined, env);
  } catch (error) {
    console.error("similar-deals-failed", { dealId, error: String(error) });
    return jsonResponse({ error: "Failed to get similar deals" }, 500, undefined, env);
  }
}
...
export async function handleD1Recommended(
  ...
  try {
    ...
    return jsonResponse({ recommended: deals, total: deals.length }, undefined, env);
  } catch (error) {
    console.error("recommended-deals-failed", { error: String(error) });
    return jsonResponse({ error: "Failed to get recommended deals" }, 500, undefined, env);
  }
}
...
export async function handleD1Trending(url: URL, env: Env): Promise<Response> {
  ...
  try {
    ...
    return jsonResponse({ error: "Failed to get trending deals" }, 500, undefined, env);
  }
}
