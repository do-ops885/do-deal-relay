...
/**
 * Parse command details from email body
 */
export function parseCommandDetails(
  ...
  return {
    service: extractField(/(?:service|company|app)\s*[:\-]?\s*(.+)/i),
    code: extractField(/(?:code|referral code|invite code)\s*[:\-]?\s*(\S+)/i),
    referralUrl: extractField(
      /(?:link|url|referral link)\s*[:\-]?\s*(https?:\/\/\S+)/i,
    ),
    expiry: extractField(/(?:expiry|expires|valid until)\s*[:\-]?\s*(.+)/i),
    reward: extractField(/(?:reward|bonus|benefit)\s*[:\-]?\s*(.+)/i),
    category: extractField(/(?:category|type)\s*[:\-]?\s*(.+)/i),
    reason: extractField(/(?:reason|note)\s*[:\-]?\s*(.+)/i),
    query: extractField(/(?:search|query|find)\s*[:\-]?\s*(.+)/i),
  };
}

/**
 * Create full command object from email
 */
export function parseCommand(email: ParsedEmail): ParsedCommand {
  const type = detectEmailType(email);
  const details = parseCommandDetails(email);
