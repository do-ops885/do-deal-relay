...
| 3 | Publish-stage matcher (free-tier path): new-deal batch x active subscriptions via D1 FTS5 + precomputed subscription-query embeddings with in-worker cosine match, `score >= threshold`; zero Vectorize queries at publish; shared threshold helpers; `deal_alert_match` compliance event | worker/lib/alerts/matcher.ts, worker/publish.ts, worker/routes/nlq/utils.ts, tests/unit/alerts/matcher.test.ts | medium |
| 4 | Queues fan-out with hard caps: `ALERT_QUEUE` producer + consumer + DLQ in wrangler; `Env` binding; `queue()` export in index; idempotent `alert_deliveries` guard; instant vs digest split; producer enforces `MAX_ALERT_QUEUE_MESSAGES_PER_PUBLISH = 500` and `MAX_MATCHES_PER_SUBSCRIPTION_PER_PUBLISH = 10` with digest rollover; binding-missing or budget-exhausted falls back to inline best-effort plus existing KV-DLQ | wrangler.jsonc, worker/types/api.ts, worker/index.ts, worker/queues/alert-consumer.ts, worker/lib/d1/alert-deliveries.ts, worker/lib/alerts/budgets.ts, tests/unit/queues/alert-consumer.test.ts | medium |
...
- [ ] Matcher runs on new-deal batch only; `score >= threshold` strict parity with shared helpers; zero sends on empty batch; zero Vectorize queries at publish (free-tier path)
- [ ] Queue producer enforces message and per-subscription match caps with digest rollover (never silent drop); payload is IDs only
- [ ] Queue consumer is idempotent (duplicate delivery skipped); DLQ receives poison messages after max retries
- [ ] Digest subscriptions accumulate in D1 keyed by run date (one queue message per digest subscription per day) and drain only on `0 9 * * *` cron; instant subscriptions enqueue immediately
...
- [ ] Vectorize free-plan availability is self-contradictory across official pricing pages (paid-only note vs published free tier). Default stays on the free-tier matcher path (D1 FTS5, zero publish-time Vectorize queries) until a dashboard check confirms free-account Vectorize calls succeed.

## Risk Assessment

| Risk | Impact | Mitigation |
|------|--------|------------|
| Fan-out storm on large publish batch | high | Matcher capped to new-deal batch; per-subscription early exit on threshold; queue decouples publish from send; hard caps (500 msgs/publish, 10 matches/sub) with digest rollover; binding missing or budget exhausted falls back to inline best-effort with warn plus KV-DLQ |
...
- [ ] Queues on free plan confirmed available since 2026-02-04 (10k ops/day); design caps keep usage near 6k ops/day worst case (see ADR-031 addendum)
- [ ] Free-tier-only constraint: no paid-only bindings; cron count frozen at 4; prod observability sampling lowered to 0.1; per-batch (never per-message) logging
