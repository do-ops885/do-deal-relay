  ...
  "localConfigTools": [
    {
      ...
      "issueCount": 42,
      "note": "Issues from this tool are controlled by the project's ESLint config, not Codacy patterns."
    }
  ],
  "codacyYaml": null,
  "importResults": null,
...
1. **Auto-import mode**: Proceed directly to 6c.
...
- Check if its parameters can be tweaked → `codacy pattern <toolName> <patternId> --parameter key=value`
...
If the import encounters Coding Standard conflicts (409 errors):

1. **If force mode is enabled**: Automatically retry with `--force`.
2. **If force mode is NOT enabled** (default): Report which tools/patterns could not be changed. Ask the user whether to unlink the Coding Standards and retry with `--force`.
