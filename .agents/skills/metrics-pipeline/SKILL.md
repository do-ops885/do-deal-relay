...
```typescript
const responseTime = metrics.summary("response_time_seconds", "Response time", {
...
```typescript
const httpRequests = metrics.counter("http_requests_total", "HTTP requests", [
  "method",
  "route",
  "status",
]);
...
```typescript
const dealValue = metrics.gauge("deal_value_dollars", "Deal value", [
  "category",
  "source",
]);
